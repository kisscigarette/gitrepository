...
public class PopupMenuUtil {
    ...
    private void _createView(final Context context) {
        ...
        rootVew.setFocusable(true); // 这个很重要
        rootVew.setFocusableInTouchMode(true);
        WindowManager manger = (WindowManager) context.getSystemService(context.WINDOW_SERVICE);
        int height = manger.getDefaultDisplay().getHeight() * 5 / 9;
        ...
        if (animatorProperty == null) {
            top = dip2px(context, 410);
            bottom = dip2px(context, 210);
            animatorProperty = new float[]{bottom, 80, -40, -20 - 10, 0};
        }
    ...
    private void _openPopupWindowAction() {
        ...
        _startAnimation(llTest1, 550, animatorProperty);
        _startAnimation(llTest2, 530, animatorProperty);
        _startAnimation(llTest3, 530, animatorProperty);
        _startAnimation(llTest4, 550, animatorProperty);

        _startAnimation(llTest5, 550, animatorProperty);
        _startAnimation(llTest6, 530, animatorProperty);
        _startAnimation(llTest7, 530, animatorProperty);
        _startAnimation(llTest8, 550, animatorProperty);
    }
